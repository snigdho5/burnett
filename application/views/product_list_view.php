...
<?php if (count(@$category_list) > 0) {
  foreach ($category_list as $row) { ?><li><span class="caret"><?php echo $row->name; ?></span>
  ...
  echo '<li>No Category Found.</li>';
      ...
      <div class="col-md-9 home_featured  mt-5">
        ...
        if ($parent_slug != '' && $sub_slug == '') {
          ...
          echo '<h4>' . $title_parent_category_details[0]->name . '</h4>';
        }
        ...
        if ($sub_slug != '') {
          ...
          echo (!empty($title_parent_category_details) && !empty($title_sub_category_details)) ? $title_parent_category_details[0]->name . ' > ' . $title_sub_category_details[0]->name : '';
        }
    ...
    <li class="page-item"><a class="page-link" href="#">Next</a></li>
        ...
        <div class="container new_list_div">
          <div class="alpha-series">
            <?php
            //snigdho
            $subcat = ($this->uri->segment(3) != null) ? strtolower($this->uri->segment(3)) : 'filter';
            foreach (range('A', 'Z') as $column) {
              if ($column == 'A') {
            ?>
                <a href="<?php echo base_url() . 'product-list/' . strtolower($this->uri->segment(2)) . '/' . $subcat . '/' . $column; ?>" class="first"><?php echo $column; ?></a>
              <?php
              } else if ($column == 'Z') {
              ?>
                <a href="<?php echo base_url() . 'product-list/' . strtolower($this->uri->segment(2)) . '/' . $subcat . '/' . $column; ?>" class="last"><?php echo $column; ?></a>
              <?php
              } else {
              ?>
                <a href="<?php echo base_url() . 'product-list/' . strtolower($this->uri->segment(2)) . '/' . $subcat . '/' . $column; ?>" class="middle"><?php echo $column; ?></a>
            <?php
              }
            }
            ?>
...
<script type="text/javascript">
  function productaddtocart(pid) {
    ...
    $('.ddlsize_id_' + pid).val(size);
    $('.variable_attribute_id_' + pid).val(variable_attribute_id);
