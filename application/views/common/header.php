...
<head>
  ...
  <meta name="keywords" content="<?php
                                  if (@$seo_content_details[0]->meta_keyword) {
                                    echo strip_tags(@$seo_content_details[0]->meta_keyword);
                                  } else {
                                    echo "Burnett Research Laboratory";
                                  } ?>" />

  <meta name="description" content="<?php echo strip_tags(@$seo_content_details[0]->meta_description); ?>">
  <title><?php if (@$seo_content_details[0]->meta_title != '') {
            echo @$seo_content_details[0]->meta_title;
          } else {
            echo 'Burnett Research Laboratory';
          } ?></title>
        ...
        <a class="nav-link" href="#">Career</a>
          ...
          <div class="col-md-4">
            <div class="top_login text-center text-md-right">
              ...
              &nbsp; <span class="the_cart_top"><a href="<?php echo base_url(); ?>cart"><i class="fa fa-shopping-cart" aria-hidden="true"></i></a> <?php echo $total_qty; ?></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <li class="nav-item">
        <a class="nav-link" href="<?php echo base_url(); ?>product-list/herbal">Herbal</a>
                ...
                foreach ($category_list as $row) {
                  ...
                  // print_obj($row);die;
                  if ($this->uri->segment(1) !== null && $this->uri->segment(1) == 'product-details' && $this->uri->segment(2) !== null) {
                    //snigdho
                    $product_id = $this->uri->segment(2);
                    //echo $product_id;exit();

                    $toGetProduct_ID = $this->db->where('unique_key', $product_id)->get('product')->result();
                    $product_id = (!empty($toGetProduct_ID)) ? $toGetProduct_ID[0]->product_id : '0';
                    ...
                    if ($product_id != 0) {
                      $product_details = $this->prom->get_product_category(array('product_category.product_id' => $product_id));
                      // print_obj($product_details);die;
                      if (!empty($product_details) && $product_details['parent_id'] > 0) {
                        $subcat_details = $this->prom->get_category(array('cat_id' => $product_details['parent_id']));
                        // print_obj($subcat_details);die;
                      }
                    }
                  }
                ?>
                  <li class="nav-item dropdown megamenu-li dmenu">
                    <a class="nav-link
                     <?php
                      if (count(@$sub_category_list) > 0 && isset($row)) {
                        echo ' dropdown-toggle';
                      } ?>" href="<?php echo base_url() . 'product-list/' . strtolower($row->unique_name); ?>" id="dropdown<?php echo $row->cat_id; ?>" <?php if (count(@$sub_category_list) > 0) { ?> data-toggle="dropdown" aria-haspopup="true" <?php } ?> aria-expanded="false" <?php

                                                                                                                                                                                                                                                                          if (isset($row) && (strtolower($this->uri->segment(2)) == strtolower($row->unique_name) || (isset($product_details) && !empty($product_details) && strtolower($product_details['unique_name']) == strtolower($row->unique_name)) || (isset($subcat_details) && !empty($subcat_details) && strtolower($subcat_details['unique_name']) == strtolower($row->unique_name)) || strtolower($this->uri->segment(2)) == strtolower($row->name))) {
