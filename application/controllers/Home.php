...
class Home extends CI_Controller 
{
	...
	public function index()
	{
		...
		$data['commitmentInfo'] = $this->db->where('status', 1)->get('commitment')->result();
		$data['imagesInfo'] 	= $this->db->get('upload_images')->result();
		$data['blogInfo'] 		= $this->db->where('status', '1')->order_by('added_date','ASC')->get('blog')->result();
