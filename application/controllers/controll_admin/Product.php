...
<?php
  ...
  public function product_bulk_upload()
  {
    ...
    if (isset($_POST['submitBtn'])) {
      $exnt = pathinfo($_FILES['bulkupload_file']['name'], PATHINFO_EXTENSION);

      if ($exnt == 'xls' || $exnt == 'xlsx') {
        $nameOfFile  = $_FILES['bulkupload_file']['tmp_name'];
        ...
        foreach ($obj->getWorksheetIterator() as $sheet) {
          ...
          if ($highestColumn == 'Q' && $totalColumNo == 17) {
            ...
            $this->session->set_flashdata('succ_msg', 'Product details uploaded successfully.');
            redirect(base_url() . BaseAdminURl . '/product/bulkupload');
            exit();
          } else {

            $this->session->set_flashdata('error_msg', 'Please upload excel sheet in given format.');
            redirect(base_url() . BaseAdminURl . '/product/bulkupload');
            exit();
          }
        }
      } else {
        $this->session->set_flashdata('error_msg', 'Invalied file type.');
        redirect(base_url() . BaseAdminURl . '/product/bulkupload');
        exit();
      }
    }
