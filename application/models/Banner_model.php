...
class Banner_model extends CI_Model {
	...
	public function edit_banner($id, $data) {
 ...
 /*public function get_sub_categories($category_id)
    ...
    public function duplicate_banner_name_chk($name) {	
		$this->db->select('*');
		$this->db->from('banner');
	...
	public function banner_details_by_id($id){
		
		$this->db->select('*');
		$this->db->from('banner');
		
		$this->db->where(array('banner_id'=>$id));
		$query = $this->db->get();
		$result = $query->result();
		if(!empty($result)){			
